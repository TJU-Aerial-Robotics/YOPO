...
namespace sdf_tools
{
    ...
    visualization_msgs::Marker SignedDistanceField::ExportForDisplay(const float alpha, float vis_level) const
    {
        ...
        for (int64_t x_index = 0; x_index < distance_field_.GetNumXCells(); x_index++)
        {
            for (int64_t y_index = 0; y_index < distance_field_.GetNumYCells(); y_index++)
            {
                for (int64_t z_index = 0; z_index < distance_field_.GetNumZCells(); z_index++)
                {
                    ...
                    if (vis_level != FLT_MAX && (location[2] > vis_level || location[2] < vis_level-0.2) )
                        continue;
    ...
    void SignedDistanceField::DisplayPCL(pcl::PointCloud<pcl::PointXYZI> &sdf_cloud, float vis_level)
    {
        sdf_cloud.points.clear();
        // Add all the cells of the SDF to the message
        double min_distance = 0.0;
        double max_distance = 0.0;
        for (int64_t x_index = 0; x_index < distance_field_.GetNumXCells(); x_index++)
        ...
        for (int64_t x_index = 0; x_index < distance_field_.GetNumXCells(); x_index++)
        {
            for (int64_t y_index = 0; y_index < distance_field_.GetNumYCells(); y_index++)
            {
                for (int64_t z_index = 0; z_index < distance_field_.GetNumZCells(); z_index++)
                {
                    ...
                    new_point.y = location[1];
                    new_point.z = location[2];
                    new_point.intensity = distance / (max_distance - min_distance);
                    sdf_cloud.points.push_back(new_point);
                }
            }
        }
    }
